    ...
    <AuthShell active="login">
      {mode === "password" ? (
        <form action={pwAction} className="flex flex-col gap-3">
          ...
          {pwState?.error && (
            <p className="text-center text-sm text-[#FF8A82]">{pwState.error}</p>
          )}
          <button
            type="button"
            onClick={() => setMode("magic")}
            className="flex items-center justify-center gap-1.5 py-1 text-sm font-bold text-accent"
          >
            <Sparkles className="h-4 w-4" />
            Receber link magico no e-mail
          </button>
        </form>
      ) : mlState?.success ? (
        ...
        <form action={mlAction} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={mlPending} className={authPrimaryBtnClass}>
            {mlPending ? (
              ...
              <>
                <Sparkles className="h-4 w-4" /> Enviar link magico
              </>
            )}
          </button>
          {mlState?.error && (
            <p className="text-center text-sm text-[#FF8A82]">{mlState.error}</p>
          )}
          <button
            type="button"
